...
class Client:
    ...
    def _parse_form(self, form: Tag | NavigableString | None):
        filtered = filter(
            lambda x: x.has_attr("value") and x["value"] is not None,
            form.find_all("input"),
        )
    ...
    async def _login(self):
        init_get = await self._session.get(
    ...
    async def get_accounts(self) -> list[Account]:
        ...
        result = list(
            map(
                lambda x: Account(
                    acc_number=x["number"],
                    id=x["link"]["url"].replace(
                        "/retail/netbank/accounts/?account=", ""
                    ),
                    name=x["displayName"],
                    balance=x["balance"],
                    funds=x["availableFunds"],
                    currency=x["balance"],
                ),
                res_json["accounts"],
            )
    ...
    async def _get_paging_key(self, acc_id: str, page: int) -> str:
        if page in self._paging:
            ...
            if len(self._paging.items()) == 0:
                res = await self._session.get(
                    f"https://www.commbank.com.au/retail/netbank/accounts/api"
    ...
    async def get_transactions(self, acc_id: str, page: int = 1):
        paging_query_arg = (
            "&pagingKey=" + await self._get_paging_key(acc_id, page) if page > 1 else ""
        )
        res = await self._session.get(
                ...
                map(
                    lambda x: Transaction(
                        ...
                        created=datetime.strptime(
                            x["createdDate"], "%Y-%m-%dT%H:%M:%S"
                        ),
                        amount=float(x["amount"]),
                        pending=True,
                    ),
                    res_json["pendingTransactions"],
                )
            )
            if "pendingTransactions" in res_json
            else []
        )
        transactions = (
            list(
